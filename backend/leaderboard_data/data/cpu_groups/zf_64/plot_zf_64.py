...
def main():
    csv_path = os.path.join(os.path.dirname(__file__), 'zf_64.csv')
    out_path = os.path.join(os.path.dirname(__file__), 'zf_64.png')

    df = pd.read_csv(csv_path)

    round_cols = [f'round_{i}_time' for i in range(1, 17)]
    rounds = df[round_cols]
    means = rounds.mean(axis=0).values

    n = int(means.size)
    mean = float(np.mean(means))
    var_pop = float(np.var(means, ddof=0))
    std_pop = float(np.std(means, ddof=0))
    var_sample = float(np.var(means, ddof=1)) if n > 1 else 0.0
    std_sample = float(np.std(means, ddof=1)) if n > 1 else 0.0
    val_min = float(np.min(means))
    val_max = float(np.max(means))

    # plotting style matching zf_amd
    try:
    ...
    title = os.path.basename(os.path.dirname(__file__)).replace('_', ' ').upper() + ' 16 round normal distribution'
...
def main():
    ...
    # plotting: match style used by zf_amd / zf_65 plot
    try:
        plt.style.use('seaborn')
    except Exception:
        pass
    ...
    ax.set_xlabel("Round runtime (s)")
    ax.set_ylabel("Density")
    ax.set_title("Intel Xeon Gold 5120T 16 round normal distribution", fontsize=14, fontweight="bold")
